...
variable "parameter_group" {default=null}
variable "old_parameter_group" {default=null}
...
data "aws_db_instance" "replica_source_db" {
...
resource "aws_iam_role_policy_attachment" "em_role_policy_attachment" {
    count = var.enhanced_monitoring_role != null ? 1:0
    role = aws_iam_role.em_role[0].name
...
resource "aws_db_parameter_group" "old_parameter_group" {
  count       = var.old_parameter_group != null ? 1 : 0
  name        = var.old_parameter_group.name
  family      = var.old_parameter_group.family
  description = var.old_parameter_group.description
...
resource "aws_db_event_subscription" "event_subscriptions" {
    for_each = data.aws_sns_topic.sns_topics
...
resource "aws_db_instance" "rds_instance" {
  ...
  kms_key_id                   = try(data.aws_kms_key.kms_key[0].arn, null)
  ...
  monitoring_role_arn          = try(var.rds_instance.monitoring_role_arn, aws_iam_role.em_role[0].arn, null)
  ...
  password                     = try(random_password.password[0].result, null)
  ...
  replicate_source_db          = try(var.rds_instance.replicate_source_db, null) != null ? var.rds_instance.replicate_source_db : var.replica_source != null ? data.aws_db_instance.replica_source_db[0].db_instance_arn : null
